...
describe("Gas", function() {
  ...
  describe("Refunds", function() {
    ...
    it("accounts for Rsclear Refund in gasEstimate", async() => {
      ...
      return estimateGasInstance.methods
        .reset()
        .send(options) // prime storage by making sure it is set to 0
        .then(() => {
    ...
    it("accounts for Rsclear and Rselfdestruct Refunds in gasEstimate", async() => {
      ...
      return estimateGasInstance.methods
        .reset()
        .send({ from, gas: 5000000 }) // prime storage by making sure it is set to 0
        .then(() => {
  ...
  describe("Estimation", function() {
    ...
    function testTransactionEstimate(contractFn, args, options) {
      ...
      return estimateGasInstance.methods
        .reset()
        .send({ from: options.from, gas: 5000000 }) // prime storage by making sure it is set to 0
        .then(() => {
    ...
    it("matches usage for complex function call (add)", function() {
      this.timeout(10000);
      return testTransactionEstimate(estimateGasInstance.methods.add, [toBytes("Tim"), toBytes("A great guy"), 10], {
        from: accounts[0],
        gas: 3141592
      });
    });
    ...
    it("matches usage for complex function call (transfer)", function() {
      this.timeout(10000);
      return testTransactionEstimate(
      ...
      return w3.eth
        ...
        .then((balance) => {
          ...
          assert(
            ...
            `Gas price used by EVM (${to.hex(actualGasPrice)}) was different from expected gas price (${to.hex(
              expectedGasPrice
            )})`
          );
        });
