  ...
  create: function(options) {
    ...
    server.close = function() {
      ...
      if (connectionManager) {
        promises = Object.values(connectionManager.connections).map(({ connection }) => {
          return new Promise(resolve => {
            connection.once("close", () => {
              // delete the socket from our server's connection list
              // so we don't try to close it again below
              if (connection.socket.key !== null) {
                delete connections[connection.socket.key];
              }
              resolve();
            });
      ...
      promises = Promise.all(promises).then(() => {
        return Promise.all(Object.values(connections).map(connection => {
          return new Promise(resolve => {
