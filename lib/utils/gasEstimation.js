...
module.exports = (vm, runArgs, err, callback) => {
  ...
  vm.runTx(runArgs, function(vmerr, result) {
    if (vmerr || !steps.ops.length || steps.ops.length - 1 === steps.lowestGasIndex()) {
      ...
      if (steps.systemOps.length > 1 && steps.systemOps[steps.systemOps.length - 2][2] === "CALL") {
        const neededAfterCall =
          2300 - sumRange(steps.systemOps[steps.systemOps.length - 2][0] + 1, steps.ops.length - 1);
