...
module.exports = (vm, runArgs, err, callback) => {
  ...
  /**
   * Returns the sum of all costs in a system operation.  Searching outward from a deepest point
   *    to depth 0.
   * opcode fees, memory expansion costs, return costs, etc
   * @param {Number} start Beginning index
   * @param {Number} end Ending index
   * @param {Number} lowest Lowest gas point, pivot.
   */
  // const sumSystemOp = (lowest) => {
  //   // Find the range of indexes for the outer most system operation
  //   // TODO: Assert depth > 0
  //   let total = 0;
  //   let depth = steps.ops[lowest].depth;
  //   while (depth) {
  //     const { begin, end } = findRootScope(lowest);
  //     //  CREATE/CALL/DELEGATECALL - TODO: Assert op must be a System op:
  //     const callingSysOp = steps.ops[begin];

  //     // Cost of the system operation itself
  //     const callingFee = callingSysOp.opcode.fee;

  //     // Sum of the current calling system operation (after op fee)
  ...
  //     total += neededBefore6364 + callingFee;
  ...
  const getTotal = (start, stop, lowestIndex = null) => {
    ...
    while (rootBegin !== parentBegin && rootEnd !== parentEnd) {
      ...
      const remaining = sumRange(prevParentEnd, parentEnd) + costOfParentReturn;
    ...
    // Sum of the remaining steps after the current (outer) system operation
    const remaining = sumRange(rootEnd, stop);
    if (oneSixtyFloorth > remaining) {
    ...
    return total + getTotal(rootEnd, stop);
  };
};
