    ...
    if (vmerr) {
      ...
      if (opsLength) {
        ...
        if (steps.success() && lastSVT > 0) {
          const tailCost = sumRange(lastSVT, steps.ops.length - 1);
          estimate += tailCost < 2300 ? 2300 : tailCost;
        }
      }
      result.gasEstimate = new BN(estimate - 1);
    } else {
      ...
      result.gasEstimate = new BN(total - 1);
    }
  ...
  const getTotal = (start, stop, lowestIndex = null) => {
    if (lowestIndex) {
      ...
      if (lowestIndex === stop) {
        ...
        const lastSVT = steps.lastSVT();
        if (lastSVT > start && lastSVT < stop) {
          const top = sumRange(start, lastSVT);
          const tail = sumRange(lastSVT, stop);
          const needed = tail < 2300 ? 2300 : tail;
          return top + needed;
        }
    ...
    while (rootBegin !== parentBegin && rootEnd !== parentEnd) {
      ...
      const stipend = callingSysOp.opcode.name === "CALL" ? 2300 : 0;
      ...
      const neededBefore6364 = sumOfCurrentSysOp + Math.floor(sumOfCurrentSysOp / 63.00000001) + stipend;
    ...
    // What we need before the 1/64 is held back
    const neededBefore6364 = systemOpSum + Math.floor(systemOpSum / 63.00000001) + stipend;
