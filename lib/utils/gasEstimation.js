...
module.exports = (vm, runArgs, err, callback) => {
  ...
  vm.runTx(runArgs, function(vmerr, result) {
    ...
    if (vmerr || !opsLength || opsLength - 1 === steps.lowestGasIndex()) {
      let estimate = parseInt(`0x${result.gasUsed.toString("hex")}`);
      if (opsLength) {
        const lastOperation = steps.ops[opsLength - 1].opcode.name;
        const success = lastOperation === "STOP" || lastOperation === "RETURN";
        if (success && steps.simpleValueTransfers.length) {
...
const stepTracker = () => {
  ...
  const allOps = [];
  const svt = [];
  const isCall = (opname) => ["CALL", "DELEGATECALL", "STATICCALL"].includes(opname);
