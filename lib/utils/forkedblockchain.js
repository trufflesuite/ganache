...
ForkedBlockchain.prototype.getVm = function() {
  var vm = BlockchainDouble.prototype.getVm.apply(this)
  ...
  vm.stateManager.cache._lookupAccount = this.getAccount.bind(this);
  vm.stateManager.getContractCode = this.getCode.bind(this);
  vm.stateManager.putContractCode = this.putCode.bind(this);
  ...
  this.stateTrie.get(address, function(err, data) {
    ...
    account.setCode(self.stateTrie, value, function(err, result) {
      ...
      self.stateTrie.put(address, account.serialize(), function(err) {
        ...
        vm.stateManager._putAccount(address, account, callback);
      });
    });
  })
};
