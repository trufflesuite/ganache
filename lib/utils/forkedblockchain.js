...
ForkedBlockchain.prototype.getFallbackBlock = function(numberOrHash, cb) {
  ...
  self.web3.eth.getBlock(numberOrHash, true, function(err, json) {
    ...
    (json.transactions || []).forEach(function(txJson, index) {
      // const v = txJson.v - 27;
      const chainId = Math.floor((txJson.v - 35) / 2);

      const common = Common.forCustomChain(
        "mainnet", // TODO needs to match chain id
        {
          name: "ganache",
          networkId: self.forkVersion,
          chainId: chainId >= 0 ? chainId : 1337, // defualt for priv networks per: EIP155
          comment: "Local test network",
          bootstrapNodes: []
        },
        self.options.hardfork
      );
      block.transactions.push(Transaction.fromJSON(txJson, Transaction.types.real, common));
    });
...
ForkedBlockchain.prototype.getTransaction = function(hash, callback) {
  ...
  BlockchainDouble.prototype.getTransaction.call(this, hash, function(err, tx) {
    ...
    // const data = JSON.stringify({"id":1337,"jsonrpc":"2.0","method":"eth_getTransactionByHash","params":[hash]});
    // const options = {
    //   hostname: 'localhost',
    //   port: 21345,
    //   path: '/',
    ...
    self.web3.eth.getTransactionFromBlock(hash, 2, function(err, result) {
      if (err) {
        return callback(err);
      }
