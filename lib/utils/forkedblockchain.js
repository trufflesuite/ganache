...
ForkedBlockchain.prototype.patchVM = function(vm) {
  ...
  vm.stateManager._lookupStorageTrie = this.getLookupStorageTrie(trie);
...
ForkedBlockchain.prototype.createForkedStorageTrie = function(stateTrie, address) {
  ...
  var trie = stateTrie.copy();

  this.storageTrieCache[address] = trie;
  return trie;
};

ForkedBlockchain.prototype.getLookupStorageTrie = function(stateTrie) {
  return (address, callback) => {
    ...
    this.getLookupAccount(stateTrie)(address, (err, account) => {
      if (err) {
        return callback(err);
      }
...
ForkedBlockchain.prototype.getStorage = function(address, key, number, callback) {
  this.getLookupStorageTrie(this.stateTrie)(address, function(err, trie) {
    if (err) {
      return callback(err);
    }
...
ForkedBlockchain.prototype.fetchCodeFromFallback = function(address, blockNumber, callback) {
  ...
  this.getSafeFallbackBlockNumber(blockNumber, function(err, safeBlockNumber) {
    ...
    if (safeBlockNumber === 0 || address.indexOf("000000") > -1 || address === "0x00") {
