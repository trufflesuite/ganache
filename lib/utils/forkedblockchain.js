...
ForkedBlockchain.prototype.createVMFromStateTrie = function() {
  ...
  // if we haven't yet create the global storage trie cache create it now and patch the vm using it
  if (this.storageTrieCache == null) {
    this.storageTrieCache = new Map();
    this.patchVM(vm, this.storageTrieCache);
  } else {
    this.patchVM(vm);
  }
...
ForkedBlockchain.prototype.patchVM = function(vm, cache) {
  // Unfortunately forking requires a bit of monkey patching, but it gets the job done.
  const trie = vm.stateManager._trie;
  vm.stateManager._lookupStorageTrie = this.getLookupStorageTrie(trie, cache);
  vm.stateManager._cache._lookupAccount = this.getLookupAccount(trie);
  vm.stateManager.getContractCode = this.getLookupCode(trie);
};
...
ForkedBlockchain.prototype.getLookupStorageTrie = function(stateTrie, cache) {
  // use the provided storageTrieCache or clone the global cache.
  const globalStorageTrieCache = this.storageTrieCache;
  const storageTrieCache = cache || new Map();

  return function _lookupStorageTrie(address, callback) {
    address = to.hex(address);
    console.log(address);

    if (storageTrieCache.has(address)) {
      return callback(null, storageTrieCache.get(address));
    }
    // if we don't have it locally, check the global cache
    let storageTrie;
    if (globalStorageTrieCache.has(address)) {
      // if we have it in the global cache create a copy of it
      storageTrie = globalStorageTrieCache.get(address).copy();
    } else {
      storageTrie = this.createForkedStorageTrie(stateTrie, address);
      storageTrieCache.set(address, storageTrie);
    }
...
ForkedBlockchain.prototype.getLookupCode = function(trie) {
  const self = this;
  return (address, callback) => {
    // If the account doesn't exist in our state trie, get it off the wire.
    trie.keyExists(address, function(err, exists) {
      if (err) {
        return callback(err);
      }
      if (exists) {
        trie.get(address, (err, data) => {
          ...
          account.getCode(trie, callback);
        });
      } else {
        self.fetchCodeFromFallback(address, to.number(self.forkBlockNumber), callback);
      }
    });
  };
};
  ...
  return (address, callback) => {
    ...
    trie.keyExists(address, function(err, exists) {
      ...
      if (exists) {
        ...
        self.fetchAccountFromFallback(address, to.number(self.forkBlockNumber), callback);
      }
    });
  };
};
