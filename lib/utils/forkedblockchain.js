...
function ForkedBlockchain(options) {
  ...
  if (typeof options.fork === "string") {
    const blockNumber = options.fork.match(blockNumberReg);

    if (blockNumber) {
      options.fork = options.fork.replace(blockNumber, "");
      options.fork_block_number = blockNumber;
    }
