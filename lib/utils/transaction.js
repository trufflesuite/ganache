...
function fixProps(tx, data) {
  ...
  // in order to differentiate between a nonce that isn't set and a nonce is
  // set to 0 in a fake transaction.
...
function initData(tx, data) {
  ...
  // Set chainId value from the data, if it's there and the data didn't contain
  // a `v` value with chainId in it already.
  if (!tx._chainId && data) {
    tx._chainId = data.chainId || 0;
  }
}
