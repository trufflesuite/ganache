...
ConnectionManager.prototype._subscribe = function (connectionId, payload, cb) {
  ...
  switch (payload.params[ 0 ]) {
    ...
    case 'newPendingTransactions':
      ...
      break;
    case 'newHeads':
      var id = to.hex('heads_' + this.connections[ connectionId ].subscriptions.length);
...
ConnectionManager.prototype._updateSubscriptions = function (block) {
  ...
  for (var idxC = 0; idxC < this.connections.length; idxC++) {
    var c = this.connections[ idxC ];
    ...
    for (var idxS = 0; idxS < c.subscriptions.length; idxS++) {
      ...
      // 0x68656164735f is heads_ in hex. This is the prefix for all newHeads subscription ids
      if (subscription.startsWith('0x68656164735f')) {
