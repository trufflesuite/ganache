...
StateManager.prototype.queueTransaction = function(method, txParams, blockNumber, callback) {
  ...
  if (method === "eth_sendTransaction" && this.unlocked_accounts[txParams.from] == null) {
    const msg = this.accounts[txParams.from] == null ? "sender account not recognized" : "signer account is locked";
  ...
  try {
    ...
    //   const privateKey = this.accounts[txParams.from].secretKey;
    //   // if ("0x" + utils.privateToAddress(privateKey).toString("hex") !== txParams.from ) {
    //   //   throw new Error(`Private key does not match address: ${txParams.from}`); // Sanity check
    //   // }
    //   tx.sign(privateKey);
    //   txParams.v = tx.v;
    //   txParams.r = tx.r;
