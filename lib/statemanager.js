...
StateManager.prototype.processNextAction = function(override) {
  ...
  if (typeof queued.method === "function") {
    ...
    this.blockchain.getStorage(this.blockchain.vm, queued.address, queued.position, queued.block, function(
      err,
      result
    ) {
      if (err) {
    ...
    this.processTransaction(queued.from, queued.tx, "latest", intermediary);
...
StateManager.prototype.processTransaction = function(from, tx, blockNumber, callback) {
  var self = this;
...
StateManager.prototype.createTransactionWithCorrectNonce = function(tx, from, blockNumber, callback) {
  ...
  this.blockchain.getQueuedNonce(from, blockNumber, (err, expectedNonce) => {
    if (err) {
      return callback(err);
    }
