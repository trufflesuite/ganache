...
StateManager.prototype.queueRawTransaction = function(data, callback) {
  const tx = new Transaction(data, Transaction.types.signed, { common: this.blockchain.vm.opts.common });
...
StateManager.prototype.processGasEstimate = function(from, tx, blockNumber, callback) {
  ...
  function next(err, tx) {
    ...
    self.blockchain.estimateGas(tx, blockNumber, function(err, results) {
      ...
      var result = "0x";
      if (!results.error) {
        const gasRefund = results.execResult.gasRefund;
        result = gasRefund ? to.hex(results.gasEstimate.add(gasRefund)) : to.hex(results.gasEstimate);
      } else {
