...
StateManager.prototype.queueRawTransaction = function(rawTx, callback) {
  ...
  var tx = new Transaction(data, Transaction.types.real);
  // var txParams = {
  ...
  this._queueTransaction("eth_sendRawTransaction", tx, null, callback);
};
...
StateManager.prototype.queueTransaction = function(txJsonRpc, blockNumber, callback) {
  ...
  if (this.unlocked_accounts[from] == null) {
    const msg = this.accounts[from] == null ? "sender account not recognized" : "signer account is locked";
    return callback(new TXRejectedError(msg));
  }

  // we don't really trust

  const type = this.accounts[from] ? Transaction.types.real : Transaction.types.fake;
  const tx = Transaction.fromJSON(txJsonRpc, type);
  this._queueTransaction("eth_sendTransaction", tx, blockNumber, callback);
};

StateManager.prototype._queueTransaction = function(method, tx, blockNumber, callback) {
  // use toLowerCase() to properly handle from addresses meant to be validated.
  tx.from = utils.addHexPrefix(tx.from).toLowerCase();
  ...
  if (tx.gas != null) {
    rawTx.gasLimit = utils.addHexPrefix(tx.gas);
  }

  if (tx.gasPrice != null) {
    rawTx.gasPrice = utils.addHexPrefix(tx.gasPrice);
  } else {
    rawTx.gasPrice = this.gasPriceVal;
  }

  if (tx.to != null) {
    rawTx.to = utils.addHexPrefix(tx.to);
  }

  if (tx.value != null) {
    rawTx.value = utils.addHexPrefix(tx.value);
  }

  if (tx.data != null) {
    rawTx.data = utils.addHexPrefix(tx.data);
  }

  if (tx.nonce != null) {
    rawTx.nonce = utils.addHexPrefix(tx.nonce);
  }
  ...
  var self = this;
  var address = utils.toBuffer(tx.from);
  ...
  try {
    const tx = new Transaction(rawTx, Transaction.types.fake);
    tx.from = address;
  } catch (err) {
    return callback(err);
  }

  if (tx.v != null && tx.s != null && tx.v != null) {
    rawTx.v = utils.addHexPrefix(tx.v);
    rawTx.r = utils.addHexPrefix(tx.r);
    rawTx.s = utils.addHexPrefix(tx.s);
  }
