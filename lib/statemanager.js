...
StateManager.prototype.queueRawTransaction = function(rawTx, callback) {
  var tx = new Transaction(rawTx, Transaction.types.real);
...
StateManager.prototype.queueTransaction = function(method, txJsonRpc, blockNumber, callback) {
  ...
  const type = account == null ? Transaction.types.fake : Transaction.types.real;
  let tx;
  try {
...
StateManager.prototype.createTransactionWithCorrectNonce = function(tx, from, callback) {
  ...
  self.blockchain.getQueuedNonce(from, (err, expectedNonce) => {
    ...
    if (tx.type === Transaction.types.real) {
      const account = this.accounts[from];
      if (account) {
        tx.sign(account.secretKey);
      }
    } else {
      tx.sign(Buffer.from(utils.stripHexPrefix(from), "hex"));
    }
