...
StateManager.prototype.initialize = function(callback) {
  ...
  if (self.options.accounts) {
    ...
    if (self.total_accounts == null || self.total_accounts < 0) {
      // because of where this call happens in the initialization process
      // calling the callback with an `err` now will cause the engine to never initialize
      // because self.blockchain.initialize required `accounts`, which we can't create.
      // ...Sso all pending RPC calls that would normally return the err will never
      // actually get called and ganache will hang with no status.
      // So we throw.
  ...
  // if we don't have a coinbase account just use the 0x0 address.
  // not sure if this is right. Is there a right? If we can't
  self.coinbase = accounts[0] ? to.hex(accounts[0].address) : `0x${"0".repeat(40)}`;
  self.accounts = {};
