...
StateManager.prototype.queueRawTransaction = function(data, callback) {
  ...
  this._queueTransaction("eth_sendRawTransaction", tx, tx.from, null, callback);
};
...
StateManager.prototype._setTransactionDefaults = function(tx) {
  ...
  // TODO: should we allow for gasPrice to be specified as `0`?
  // Right now etheruemjs-tx doesn't allow this (it treats as empty)
...
StateManager.prototype.createTransactionWithCorrectNonce = function(tx, from, callback) {
  ...
  this.blockchain.getQueuedNonce(from, (err, expectedNonce) => {
    ...
    // ethereumjs-tx removes nonce values that are equal to 0 on both
    // "Real" transaction and "Fake" transactions. It signs these transactions
    // with the fake values, so we don't want to actually change the value
    // of the nonce as that will mess up the msghash.

    // If the transaction is _not_ a fake transaction (which implies it is a
    // rawSignedTransaction) and it's nonce is empty, then what the transaction
    // probably meant to have was a `0` value instead!
    let actualNonce;
    if (tx.isSigned()) {
      if (tx.nonce.length === 0) {
        actualNonce = Buffer.from([0]);
      } else {
        actualNonce = tx.nonce;
      }
      validateNonce(actualNonce, expectedNonce)
        .then(done)
        .catch(callback);
    } else {
      // we need to sign transactions if they haven't been signed already:
      const sign = () => {
        // fake transactions are transactions where we don't know the account's
        // private key, so we sign them with the tx's `from` address instead:
        if (tx.isFake()) {
          // so we just don't sign them?
          // sign with keccak256 to create fake private key with the right length
          tx.sign(utils.keccak256(from));
        } else {
      ...
      if (tx.nonce.length === 0) {
        ...
        validateNonce(tx.nonce, expectedNonce)
          .then(sign.bind(this, tx))
          .then(done)
          .catch(callback);
      }
    }
