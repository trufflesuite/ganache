  ...
  var runCall = function(tx, changeRoot, err, block) {
    ...
    self.createBlock(block, function(err, block) {
      ...
      self.vm.runTx(runArgs, function(vmerr, result) {
        var firstGasEsimate = result.gasRefund ? result.gasUsed.add(result.gasRefund) : result.gasUsed;
        var runningTotal = new BN(tx.getBaseFee());
        var additionalGasNeeded = new BN(0);
        ops.forEach((info, i) => {
          if (info.opcode.name === "CREATE") {
            const otherCosts = info.gasCost.subn(info.opcode.fee);
            const addit = firstGasEsimate.sub(runningTotal.add(otherCosts)).divn(64);
            additionalGasNeeded.iadd(addit);
            runningTotal.iadd(info.gasCost);
          } else {
            if (info.depth === 0) {
              runningTotal.iadd(info.gasCost);
            }
        ...
        var total = runningTotal.toNumber();
        console.log(total);
        console.log(runningTotal.add(additionalGasNeeded).toNumber());
