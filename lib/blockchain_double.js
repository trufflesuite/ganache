    ...
    self.createBlock(block, function(err, block) {
      ...
      self.vm.runTx(runArgs, function(vmerr, result) {
        if (ops.length === 0 || ops.length - 1 === lowestGasIndex) {
          ...
          const total = getTotal(lowestGasIndex, 0, ops.length - 1) + dataCostsPlusBaseFee;
          result.gasEstimate = new utils.BN(total);
        }

        function getTotal(lowestIndex, start, stop) {
          if (lowestIndex === null) {
          ...
          const callingSysOp = ops[begin - 1]; // Assert - must be a System op: CREATE/CALL/etc
          const callingFee = callingSysOp.opcode.fee;
          const currentOneSixtyFourth = callingSysOp.gasLeft - callingFee - ops[begin].gasLeft;

          // use context to calculate
          const returnCost = ops[end].gasLeft - ops[end + 1].gasLeft + currentOneSixtyFourth;
          let total = sumRange(begin, end); // Cost of call/subcalls and memory expansion
          total += returnCost; // Cost of the final return  == (bytes returned) * 200

          // What we need before the 1/64 is held back
          const neededBefore6364 = Math.floor((total * 64) / 63);
          const oneSixtyFloorth = neededBefore6364 - total;
          total = neededBefore6364;
          total += callingFee;
          ...
          if (oneSixtyFloorth > remaining) {
            ...
            return total + getTotal(null, end + 1, stop) - oneSixtyFloorth;
          }
        }
        ...
        function findRange(index) {
          ...
          while (ops[range.end].depth > 0) {
            ++range.end;
          }
          while (ops[range.begin].depth > 0) {
            --range.begin;
          }
          range.begin++; // both overshoot by 1
          range.end--; // adjust in respective direction
          return range;
        }
