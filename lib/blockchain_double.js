...
BlockchainDouble.prototype.processCall = function(tx, blockNumber, callback) {
  ...
  var runCall = function(tx, changeRoot, err, block) {
    ...
    self.createBlock(block, function(err, block) {
      ...
      let lowestGasIndex = -1;
      let lowestGasDepth = 0;
      let greatestDepth = -Infinity;
      let greatestDepthIndex;
      let sum = 0;
      // function actualCost(steps, i) {
      //   let cost = 0;

      //   return cost;
      // }
      // self.vm.on("op", function(info) {
      self.vm.on("step", function(info) {
        ...
        if (lowestGasLeft > gasLeft) {
          ...
          lowestGasDepth = depth;
        }
        if (greatestDepth < depth) {
          greatestDepth = depth;
          greatestDepthIndex = ops.length;
        }
        sum += info.opcode.fee;
        ops.push(info);
      });
      ...
      self.vm.runTx(runArgs, function(vmerr, result) {
        console.log("lowestGasLeft: " + lowestGasLeft);
        console.log("lowestGasIndex: " + lowestGasIndex);
        console.log("lowestGasDepth: " + lowestGasDepth);
        console.log("greatestDepth: " + greatestDepth);
        console.log("greatestDepthIndex: " + greatestDepthIndex);
        console.log("sum: " + sum);
        // const opsDouble = [];
        // let gasLeft = ops[0].gasLeft.toNumber();
        // ops.forEach((info, i) => {
        ...
        const start = sumRange(0, 54);
        const create = sumRange(55, 71);
        const end = sumRange(72, 84);

        const total = start + create + end;
        ...
        console.log(
          ops.map(
            (val) =>
              `${val.opcode.name}: ${val.opcode.fee} ${val.gasLeft.toNumber()} ${val.gasLeft.toNumber() -
                val.opcode.fee}`
          )
        );

        const dataCosts = parseInt("0x" + runArgs.tx.gas.toString("hex")) - 21000 - ops[0].gasLeft.toNumber();
        const actual = ops[0].gasLeft.toNumber() - ops[ops.length - 1].gasLeft.toNumber() + 21000 + dataCosts;
        const difference = result.gasUsed.toNumber() - actual;

        // console.log(`Data Costs: ${dataCosts(runArgs.tx.data) === derivation}`);

        // function dataCosts(data) {
        //   let cost = 0;
        ...
        //       cost += 4;
        //     } else {
        //       console.log("dataCosts: SANITY CHECK");
        //     }
        //   }
        //   return cost;
        // }

        console.log("Total fees (starting gasLeft - ending gasLeft + 21000 + dataCosts): " + actual);
        console.log("Result.gasUsed: " + result.gasUsed.toNumber());
        console.log("Difference: " + difference);

        // let innerSum = 0;
        // let index = greatestDepthIndex;
        // let element = ops[index];
        // while (element.depth) {
        //   innerSum += ops[index].opcode.fee;
        //   element = ops[++index];
        //   // --currentDepth;
        // }
...
BlockchainDouble.prototype.processBlock = function(block, commit, callback) {
  ...
  self.vm.runBlock(
    ...
    function(vmerr, results) {
      ...
      console.log("greatestDepth: " + greatestDepth);
      console.log("greatestDepthIndex: " + greatestDepthIndex);
      console.log("sum of all opcode fee's: " + sum);
      function sumRange(start, end) {
        return ops[start].gasLeft - ops[end].gasLeft;
      }
      if (cyclops.length === 85) {
        const start = sumRange(0, 54);
        const create = sumRange(55, 71);
        const end = sumRange(72, 84);
        const total = start + create + end;
      ...
      const actual =
        cyclops[0].gasLeft.toNumber() -
        cyclops[cyclops.length - 1].gasLeft.toNumber() +
        21000 +
        dataCosts(block.transactions[0].data);
      const difference = results.results[0].gasUsed.toNumber() - actual;
      ...
      console.log("Total fees (starting gasLeft - ending gasLeft + 21000 + dataCosts): " + actual);
      console.log("Result.gasUsed: " + results.results[0].gasUsed.toNumber());
      console.log("Difference: " + difference);
      ...
      console.log(
        ops.map(
          (val) =>
            `${val.opcode.name}: ${val.opcode.fee} ${val.gasLeft.toNumber()} ${val.gasLeft.toNumber() - val.opcode.fee}`
        )
      );
      // If no error, check for a runtime error. This can return null if no runtime error.
