...
BlockchainDouble.prototype.getQueuedNonce = function(address, callback) {
  ...
  this.vm.stateManager.getAccount(addressBuffer, function(err, val) {
    if (err) {
      return callback(err);
    }
...
BlockchainDouble.prototype.processCall = function(tx, blockNumber, callback) {
  var self = this;
  var stateManager = this.vm.stateManager;

  var cleanUpAndReturn = function(err, result, startingStateRoot) {
    stateManager.revert(function(e) {
      // For defaultBlock, undo state root changes
      if (startingStateRoot) {
        stateManager.setStateRoot(startingStateRoot, () => {
  ...
  var runCall = function(tx, err, block, startingStateRoot) {
    if (err) {
      return callback(err);
    }
    ...
    self.createBlock(block, function(err, block) {
      ...
      // We checkpoint here in order to revert any state changes during the call
      // to runTx, as the current context is in an eth_call or eth_estimateGas
      // at this point.
      stateManager.checkpoint(() => {
        ...
        self.vm.runTx(runArgs, function(vmerr, result) {
          ...
          if (vmerr) {
            return cleanUpAndReturn(vmerr, err, startingStateRoot);
          }
          ...
          cleanUpAndReturn(vmerr, result, startingStateRoot);
        });
      });
    });
  };
  ...
  if (blockNumber === "latest") {
    ...
    self.getBlock(blockNumber, (err, block) => {
      if (err) {
        return callback(err);
      }
      stateManager.getStateRoot((err, startingStateRoot) => {
        if (err) {
          return callback(err);
        }
        stateManager.setStateRoot(block.header.stateRoot, (err) => {
          runCall(tx, err, block, startingStateRoot);
        });
      });
    });
  }
