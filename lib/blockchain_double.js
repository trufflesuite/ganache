  ...
  vm.runBlock(
    ...
    function(vmerr, results) {
      ...
      for (var v = 0; v < results.receipts.length; v++) {
        ...
        rcptBuffers.push(rcptBuffer);
        receipts.push(rcpt);
      }

      // let txvalidation = block.validateTransactions();
      ...
      async.eachOfSeries(
        txBuffers,
        function(tx, i, done) {
          txTrie.put(utils.rlp.encode(i), tx, done);
        },
        () => {
          async.eachOfSeries(
            rcptBuffers,
            function(rcpt, i, done) {
              rcptTrie.put(utils.rlp.encode(i), rcpt, done);
            },
            () => {
              block.header.transactionsTrie = txTrie.root;
              block.header.receiptTrie = rcptTrie.root;

              block.header.validateDifficulty = (pb) => true;
              // block.genTxTrie(() => {
              // block.txTrie.root.copy(block.header.transactionsTrie);

              block.validate(self, (args) => {
                // console.log(args); // should be empty string
                // block.header.transactionsTrie = block.txTrie.root;
                // let validation = block.validateTransactionsTrie();
