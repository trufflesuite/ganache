    ...
    self.createBlock(block, function(err, block) {
      ...
      self.vm.runTx(runArgs, function(vmerr, result) {
        ...
        if (ops.length === 0 || ops.length - 1 === lowestGasIndex) {
          ...
          console.log(lowestGasLeft);
          const callingSysOp = findRangeUp(lowestGasIndex); // should be a return
          const outerSysOp = findRangeDown(lowestGasIndex + 1);
          const start = sumRange(0, callingSysOp);
          const inner = sumRange(callingSysOp + 1, lowestGasIndex);
          // const end = sumRange(lowestGasIndex + 1, ops.length - 1);
          console.log(
            ops.map(
              (val) =>
          ...
          const the6364 = Math.floor((ops[callingSysOp].gasLeft.toNumber() - ops[outerSysOp].opcode.fee) / 64);
          const returnCost =
            ops[lowestGasIndex].gasLeft.toNumber() - ops[lowestGasIndex + 1].gasLeft.toNumber() + the6364;

          const dataCosts = parseInt("0x" + runArgs.tx.gas.toString("hex")) - 21000 - ops[0].gasLeft.toNumber();

          const improvedEstimation =
            Math.floor(((returnCost + inner) * 64) / 63) + ops[callingSysOp].opcode.fee + start + 21000 + dataCosts;
          console.log(improvedEstimation);
          result.gasEstimate = new utils.BN(improvedEstimation);
        }
        ...
        function findRangeUp(begin) {
          let i = begin;
          while (ops[i].depth >= begin.depth) {
            --i;
          }
          return i;
        }
        function findRangeDown(begin) {
          let i = begin;
          while (ops[i].depth > 0) {
            ++i;
          }
