...
BlockchainDouble.prototype.popBlock = function(callback) {
  ...
  this.data.blocks.last(function(err, block) {
    ...
    self.getBlock(block.header.parentHash, function(err, newLastBlock) {
      if (err) {
        return callback(err);
      }
...
BlockchainDouble.prototype.getQueuedNonce = function(address, blockNumber, callback) {
  var nonce = null;
  ...
  this.getData(addressBuffer, blockNumber, function(err, data) {
    if (err) {
      return callback(err);
    }
...
/**
 * Gets data stored at the address in the stateTrie at the specified blockNumber
 * @param {address} Buffer
 * @param {blockNumber} Number
 * @param {callback} Function
 */
BlockchainDouble.prototype.getData = function(address, blockNumber, callback) {
  this.getBlock(blockNumber, (err, block) => {
    if (err) {
      return callback(err);
    }
    const trie = this.createStateTrie(this.data.trie_db, block.header.stateRoot);
    trie.get(address, (err, data) => {
      callback(err, data, trie);
    });
  });
};
...
BlockchainDouble.prototype.processTransactionTrace = async function(hash, params, callback) {
  ...
  this.getTransactionReceipt(targetHash, function(err, receipt) {
    ...
    self.getBlock(targetBlock.header.parentHash, function(err, parent) {
      ...
      self.createBlock(parent, async function(err, block) {
        if (err) {
          return callback(err);
        }
        ...
        for (var i = 0; i < targetBlock.transactions.length; i++) {
          ...
          // await new Promise((resolve) => stateTrie.get(tx.from, (_, data) => {
          //   const account = new Account();
          //   console.log(to.number(account.nonce));
          //   resolve();
          // }));
...
BlockchainDouble.prototype.getAccount = function(address, blockNumber, callback) {
  this.getData(utils.toBuffer(address), blockNumber, function(err, data) {
    if (err) {
      return callback(err);
    }
...
BlockchainDouble.prototype.getStorage = function(vm, address, position, blockNumber, callback) {
  this.getData(utils.toBuffer(address), blockNumber, (err, data) => {
    if (err) {
      return callback(err);
    }

    const account = new Account(data);
    const trie = this.createStateTrie(this.data.trie_db, account.stateRoot);
    trie.get(utils.setLengthLeft(utils.toBuffer(position), 32), (err, data) => {
      callback(err, data);
    });
  });
};

BlockchainDouble.prototype.getCode = function(address, blockNumber, callback) {
  this.getData(utils.toBuffer(address), blockNumber, (err, data, trie) => {
    if (err) {
      return callback(err);
    }
