...
BlockchainDouble.prototype.processStorageTrace = function(structLog, storageStack, event, callback) {
  ...
  let handleStorageOpcodes = function () {
    switch (name) {
      case 'SSTORE':
      ...
      case 'SLOAD':
        ...
        storageStack.loadStorageKey = key
        callback(null, structLog)
  ...
  if (storageStack.loadStorageKey) {
    self.vm.stateManager.getContractStorage(event.address, args[0], function (err, result) {
      ...
      storageStack.stack[storageStack.currentDepth][storageStack.loadStorageKey] = value;

      delete storageStack.loadStorageKey
      handleStorageOpcodes();
    })
  } else {
    handleStorageOpcodes();
  }
