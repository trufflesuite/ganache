...
BlockchainDouble.prototype.createBlock = function(parent, callback) {
  ...
  getParent(function(err, parent) {
    ...
    // TODO: figure the right way to do this, and what the right genesis block
    // difficulty is. Doing it like below causes `difficulty` to be too high
    // and things can explode when something tries to serialize it from a BN
    // to an Int (the dreaded 53 bits error).
    // if (parent) {
    //   block.header.difficulty = block.header.canonicalDifficulty(parent);
    // } else {
    //   block.header.difficulty = "0x1";
    // }
...
BlockchainDouble.prototype.getQueuedNonce = function(address, callback) {
  ...
  this.vm.stateManager.getAccount(to.buffer(address), function(err, val) {
    if (err) {
      return callback(err);
    }
