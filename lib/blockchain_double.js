  ...
  self.vm.runBlock(
    ...
    function(vmerr, results) {
      ...
      async.eachOfSeries(
        txBuffers,
        (tx, i, done) => {
          const key = utils.rlp.encode(i);
          txTrie.put(key, tx, () => {
            rcptTrie.put(key, rcptTrie[i], done);
          });
        },
        () => {
          ...
          block.validate(self, (args) => {
            ...
            // if (block.validateTransactions() && block.validateTransactionsTrie()) {
            ...
            // } else {
            //   callback(new Error "Invalid Transactions"));
            // }
          });
        }
      );
