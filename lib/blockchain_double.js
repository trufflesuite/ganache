    ...
    self.createBlock(block, function(err, block) {
      ...
      self.vm.runTx(runArgs, function(vmerr, result) {
        ...
        if (ops.length === 0 || ops.length - 1 === lowestGasIndex) {
          ...
          const outerSysOp = findRange(lowestGasIndex); // should be a return
          const start = sumRange(0, outerSysOp);
          const inner = sumRange(outerSysOp + 1, lowestGasIndex);
          ...
          //   )
          // );
          const the6364 = Math.floor((ops[outerSysOp].gasLeft.toNumber() - ops[outerSysOp].opcode.fee) / 64);
          const returnCost =
          ...
          const improvedEstimation =
            Math.floor(((returnCost + inner) * 64) / 63) + ops[outerSysOp].opcode.fee + start + 21000 + dataCosts;
          result.gasEstimate = new utils.BN(improvedEstimation);
        }

        // const naiveEstimation = ops[0].gasLeft.toNumber()-ops[ops.length -1].gasLeft.toNumber() + 21000 + dataCosts;
