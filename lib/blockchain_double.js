...
BlockchainDouble.prototype.getVm = function(options) {
  var self = this;

  options = Object.assign({}, self.options, options || {});

  vm = options.vm || new VM({
...
BlockchainDouble.prototype.latestBlock = function(callback) {
  if (this._workingBlock) {
    callback(null, this._workingBlock)
  } else {
...
BlockchainDouble.prototype.latestBlockNumber = function(callback) {
  this.latestBlock(function(err, latest) {
    if (err) return callback(err);
    callback(null, to.number(latest.header.number));
  });
}
...
BlockchainDouble.prototype.getEffectiveBlockNumber = function(number, callback) {
  ...
  if (number.indexOf("0x") >= 0) {
    ...
    // TODO: fix pending block
    if (number == "latest" || number == "pending") {
      return this.latestBlockNumber(callback);
...
BlockchainDouble.prototype.putAccount = function(account, address, callback) {
  ...
  vm.stateManager._putAccount(address, account, function(err) {
    if (err) return callback(err);
 ...
 * @param  {Object} vm         The VM to use to process the block. Optional.
...
BlockchainDouble.prototype.processTransactionTrace = function(hash, params, callback) {
  ...
  function afterTx_listener() {
    if (tx_hash_currently_processing == target_hash) {
      vm.removeListener("step", step_listener);
      vm.removeListener("beforeTx", beforeTx_listener);
      vm.removeListener("afterTx", afterTx_listener);
    }
  }
  ...
  vm.on("beforeTx", beforeTx_listener);
  vm.on("afterTx", afterTx_listener);
    ...
    self.getBlock(targetBlock.header.parentHash, function(err, parent) {
      ...
      self.setWorkingBlock(parent, () => {
        ...
        self.createBlock(parent, function(err, block) {
          ...
          self.processBlock(block, false, vm, function(err, transactions, results) {
...
BlockchainDouble.prototype.setWorkingBlock = function(block, callback) {
  var self = this

  if (block) {
    this.stateTrie.sem.take(() => {
      self._workingBlock = block
      self._originalStateRoot = self.stateTrie.root
      self.stateTrie.root = block.header.stateRoot
      self.stateTrie.sem.leave()
      callback(null, block)
    })
  } else {
    this.stateTrie.sem.take(() => {
      self._workingBlock = null
      self.stateTrie.root = self._originalStateRoot
      delete self._originalStateRoot
      self.stateTrie.sem.leave()
      callback(null, null)
    })
  }
...
BlockchainDouble.prototype.clearWorkingBlock = function(callback) {
  this.setWorkingBlock(null, callback)
}
