 ...
 * @param {number} index - Requested index to uncle array
 * @callback callback - handles an array or Error
 * @throws - Uncle array index must be 0 or 1
 * @throws - Block hash not found
 * @throws - Uncle array index is out of range
 */
GethApiDouble.prototype.eth_getUncleByBlockHashAndIndex = function(hash, index, callback) {
  // Check and convert hex index value into an integer
  const _index = typeof index === "string" && index.indexOf("0x") === 0 ? parseInt(index.split("0x").pop()) : index;

  // Check for valid uncle index
  if (_index === 0 || _index === 1) {
    this.state.getBlock(hash, function(err, block) {
      let response;
      if (err) {
        response =
          err.message && err.message.indexOf("Key not found in database") >= 0
            ? [new Error("Unknown block hash")]
            : [err];
      } else {
        const { uncleHeaders } = block;
        const uncleCount = uncleHeaders.length;
        response =
          _index < uncleCount ? [null, uncleHeaders[_index]] : [new Error("Uncle array index is out of range")];
      }
      // eslint-disable-next-line standard/no-callback-literal
      callback(...response);
    });
  } else {
    callback(new Error("Uncle array index must be 0 or 1"));
  }
};
 ...
 * @param {number} index - Requested index to uncle array
 * @callback callback - handles a number or Error
 * @throws - Uncle array index must be 0 or 1
 * @throws - Block number is either negative or higher than block height
 * @throws - Invalid tag detected
 * @throws - Uncle array index is out of range
 */
GethApiDouble.prototype.eth_getUncleByBlockNumberAndIndex = function(blockNumber, index, callback) {
  // Check and convert hex index value into an integer
  const _index = typeof index === "string" && index.indexOf("0x") === 0 ? parseInt(index.split("0x").pop()) : index;
  ...
  if (_index === 0 || _index === 1) {
    this.state.getBlock(blockNumber, function(err, block) {
      ...
      // Check for VM errors
      if (err) {
        // Translate out or range errors
        response =
          err.message && err.message.indexOf("index out of range") >= 0 ? [new Error("Unknown block number")] : [err];
      } else {
        const { uncleHeaders } = block;
        const uncleCount = uncleHeaders.length;

        // Check if index exist in uncle array
        response =
          _index < uncleCount ? [null, uncleHeaders[_index]] : [new Error("Uncle array index is out of range")];
      }
      // eslint-disable-next-line standard/no-callback-literal
