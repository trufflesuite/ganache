...
GethApiDouble.prototype.eth_blockNumber = function(callback) {
  this.state.blockNumber(function(err, result) {
    ...
    callback(null, to.hexWithoutLeadingZeroes(result));
  });
};
...
GethApiDouble.prototype.eth_getBlockByNumber = function(block_number, include_full_transactions, callback) {
  this.state.blockchain.getBlock(block_number, function(err, block) {
    ...
    callback(null, {
      number: to.hexWithoutLeadingZeroes(block.header.number),
      hash: to.hex(block.hash()),
      parentHash: to.hex(block.header.parentHash),
      nonce: to.hexWithLeftPadding(block.header.nonce, 16),
      ...
      difficulty: to.hexWithoutLeadingZeroes(block.header.difficulty),
      totalDifficulty: to.hexWithoutLeadingZeroes(block.header.difficulty), // TODO: Figure out what to do here.
      extraData: to.hexWithEvenLength(block.header.extraData),
      mixHash: to.hex(block.header.mixHash),
      size: to.hexWithoutLeadingZeroes(1000), // TODO: Do something better here
      gasLimit: to.hexWithoutLeadingZeroes(block.header.gasLimit),
      gasUsed: to.hexWithoutLeadingZeroes(block.header.gasUsed),
      timestamp: to.hexWithoutLeadingZeroes(block.header.timestamp),
