...
export const ecdaRecover = (
  ...
  if (isEip155) {
    ...
    data = raw.slice(0, 6).concat([chainBuf, BUFFER_EMPTY, BUFFER_EMPTY]);
    recid = v - eip155V;
  } else {
    data = raw.slice(0, 6);
    recid = v - 27;
  ...
  const message = keccak(rlpEncode(data));
...
export const computeFromAddress = (
  ...
  const senderPubKey = ecdaRecover(SHARED_BUFFER, v, chainId, raw);
...
export const computeHash = (raw: EthereumRawTx) => {
  return Data.from(keccak(rlpEncode(raw)), 32);
};
...
export const computeInstrinsics = (
  ...
  return {
    from: computeFromAddress(v.toNumber(), raw, chainId),
