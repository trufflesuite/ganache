import {Data, IndexableData} from "@ganache/utils";
...
class TraceData {
  ...
  constructor(value: Buffer) {
    if (value.length > TraceData.BYTE_LENGTH) {
      throw new Error(`TraceData can only store up to ${TraceData.BYTE_LENGTH} bytes`);
    }
  ...
  public toString(): string {
    ...
    if (padBy < 0) {
      ...
      throw new Error("Improperly truncating trace data during stringification!");
    } else if (padBy > 0) {
