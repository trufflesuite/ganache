...
describe("database", () => {
  ...
  function startProvider() {
    return getProvider({ database: { dbPath }, wallet: { deterministic: true } })
  }
  ...
  it("resumes blockchain from a prior state", async () => {
    ...
    const startingBalance = await provider.request({ method: "eth_getBalance", params: [from] });
    await provider.request({ method: "eth_sendTransaction", params: [{ from, to, value: "0xffffffffff" }] })
    const endingBalance = await provider.request({ method: "eth_getBalance", params: [from] });
    ...
    const currentBalanceBeforeRevert = await provider.request({ method: "eth_getBalance", params: [from] });
    assert.strictEqual(currentBalanceBeforeRevert, endingBalance);

    const currentBlockBeforeRevert = await provider.request({ method: "eth_blockNumber", params: [] });
    ...
    await provider.request({ method: "eth_sendTransaction", params: [{ from, to, value: "0xffffffffff" }] })
    const endingBalanceAfterSnapshot = await provider.request({ method: "eth_getBalance", params: [from] });
    const latestBlockAfterSnapshot = await provider.request({ method: "eth_blockNumber", params: [] });
    ...
    const endingBalanceAfterRevert = await provider.request({ method: "eth_getBalance", params: [from] });
    const latestBlockAfterRevert = await provider.request({ method: "eth_blockNumber", params: [] });
    ...
    const balanceAfterRestart = await provider.request({ method: "eth_getBalance", params: [from] });
    assert.strictEqual(balanceAfterRestart, endingBalance);

    const latestBlockAfterRestart = await provider.request({ method: "eth_blockNumber", params: [] });
