  ...
  describe("eth", () => {
    describe("getProof", () => {
      ...
      beforeEach(async () => {
        ...
        const contractAccountBalance = await provider.send("eth_getBalance", [contractAccount.address.toString()]);
        ...
        contractAccount.codeHash = Buffer.from("224b4f9317e44dbf233c853aa45f89db2f5b76989444a324ae9475fbed1d80a0", "hex");
        contractAccount.stateRoot = Buffer.from("fe26ce2b5dafd303bdbd9adbd1bbc1ef59fceea4f81c282158ab624e2fd1787d", "hex");
      });
      ...
      it("gets the proof for an externally owned address", async () => {
        ...
        assert.equal(result.codeHash, Data.toString(KECCAK256_NULL), "Unexpected codeHash, should have been keccak hash of zero bytes");
      ...
      it("gets the proof with specific storage index", async () => {
        ...
        assert.deepEqual(result.storageProof, expectedStorageProof, "Unexpected storageProof");
      });
      ...
      it("throws with a forked network", async () => {
        ...
        try {
          [
            ...
            const error = "eth_getProof is not supported on a forked network. See https://github.com/trufflesuite/ganache/issues/3234 for details.";
            assert.rejects(forkedProvider.send("eth_getProof", [contractAccount.address.toString(), [], "latest"]), error);
          });
        } finally {
